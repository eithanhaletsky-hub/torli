        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold">שיעור חדש</h2>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <div className="space-y-2 text-sm mb-4">
              <p><strong>מקצוע:</strong> {selectedApt.service.name}</p>
            ...
            <a
              href={`https://wa.me/972${selectedApt.clientPhone.replace(/[-\s]/g, "").replace(/^0/, "")}?text=${encodeURIComponent(
                `שלום ${selectedApt.clientName} 📚\nתזכורת: יש לך שיעור ${selectedApt.service.name} בתאריך ${new Date(selectedApt.date).toLocaleDateString("he-IL")} בשעה ${new Date(selectedApt.date).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit" })}.\nנתראה! 😊`
              )}`}
              target="_blank"
